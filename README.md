...
5. [Quadratic programming model](#quadratic-programming-qp-model)
...
I will focus on the most simple execution of each command. However, for the steps in which the same command is generally run for multiple files (cleaning, annotations, assemblies, etc.). If you want to run the commands on many samples at once, you can use bash scripts to perform these steps in batch, which is more useful and closer to how we run these analyses. They are especially helpful if you want to submit jobs on a computing cluster. Some examples of bash scripts are found in the [scripts](/scripts) folder within this repository.
